//Sandwiches: Write a function that accepts a array of items a person wants on a sandwich. The function should have one parameter that collects as many items as the function call provides, and it should print a summary of the sandwich that is being ordered. Call the function three times, using a different number of arguments each time

// Function to summarize a sandwich order
function order_sandwich(...toppings: string[]): void {
    console.log("Sandwich Summary:");
    
    if (toppings.length > 0) {
        console.log("You ordered a sandwich with the following toppings:");
        for (const topping of toppings) {
            console.log(`- ${topping}`);
        }
    } else {
        console.log("Looks like you forgot to choose any toppings. Your sandwich is plain.");
    }

    console.log("Enjoy your sandwich!");
}

// Call the function with different numbers of arguments
order_sandwich('egg', 'Cheese', 'Lettuce');
order_sandwich('Turkey', 'Tomato');
order_sandwich(); // No toppings provided